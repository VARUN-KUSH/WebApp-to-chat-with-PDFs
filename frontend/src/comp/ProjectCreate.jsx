    ...
    <div className="max-w-xl mx-auto mt-8 p-4 bg-white shadow-md rounded">
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="mb-4">
          ...
          <div {...getRootProps({ className: 'dropzone mt-1 block w-full px-3 py-2 border-2 border-dashed border-gray-300 rounded-md shadow-sm focus:outline-none' })}>
            <input {...getInputProps()} />
            {pdfFile ? <p>{pdfFile.name}</p> : <p>Drag 'n' drop a PDF file here, or click to select one</p>}
          </div>
        </div>
        {message && (
